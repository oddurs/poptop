...
//! order and in either framing — and the reader must find exactly the whole
//! ones.
//!
...
//!
//! Exactly the whole ones, for entries framed as `append` frames them now. An
//! entry framed as they were before checksums can still come back when it was
//! torn, in one case: when the bytes written after it are themselves torn and
//! happen to fill exactly the length it claimed. Nothing in such an entry tells
//! the two apart; that is what the checksum is for (0100).
#![no_main]
...
fuzz_target!(|plan: &[u8]| {
    // Two bytes an entry: whether it is torn, its framing, and where it is
    // cut. Its CPU figure is its position, so what comes back says which
    // entries survived.
    let mut day = Vec::new();
    let mut whole = Vec::new();
    let mut may_linger = Vec::new();
    ...
    let mut rest = got.iter().filter(|c| !may_linger.contains(c));
    assert!(

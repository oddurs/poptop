...
/// The website draws poptop's braille again, in JavaScript, because the
/// landing page's whole argument is that a screenshot cannot show what this
/// tool does. Two implementations of the encoding that *is* the product's
/// visual identity, and until this fixture existed nothing compared them: change
/// the dot order here and the site would go on drawing the old poptop.
///
/// Writes every cell the encoding can produce — five left heights by five right
/// heights — for `web/` to check its own table against. Same workflow as the
/// theme files:
///
///     cargo test -- --ignored write_braille_fixture
///
/// `the_braille_fixture_matches_the_encoding` fails until you do.
#[test]
#[ignore]
fn write_braille_fixture() {
    use crate::glyphs::GlyphSet;
    let mut out = String::new();
    out.push_str("# Every cell poptop's braille encoding can produce: left height,\n");
    out.push_str("# right height, and the character. Generated by the\n");
    out.push_str("# `write_braille_fixture` test — see it for why this file exists.\n");
    for left in 0..=4 {
        for right in 0..=4 {
            out.push_str(&format!(
                "{left} {right} {}\n",
                GlyphSet::Braille.glyph(left, right)
            ));
        }
    }
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("web/fixtures/braille.txt");
    std::fs::create_dir_all(path.parent().unwrap()).unwrap();
    std::fs::write(path, out).unwrap();
}

/// The fixture is only useful if it is current.
#[test]
fn the_braille_fixture_matches_the_encoding() {
    use crate::glyphs::GlyphSet;
    let fixture = include_str!("../web/fixtures/braille.txt");
    let mut checked = 0;
    for line in fixture.lines().filter(|l| !l.starts_with('#')) {
        ...
        let glyph: char = parts.next().unwrap().chars().next().unwrap();
        assert_eq!(
            GlyphSet::Braille.glyph(left, right),
            glyph,
            "the encoding changed; run `cargo test -- --ignored write_braille_fixture`"
        );
        checked += 1;
    }
    assert_eq!(checked, 25, "the fixture does not cover every cell");
}
